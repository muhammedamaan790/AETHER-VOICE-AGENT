...
# THE RUNNING SYSTEM WRITES TO A WORKING COPY, NOT TO THE COMMITTED FILE. Bookings are real writes,
# and the shipped database is the hotel as every document describes it and as the demo sheet quotes
# it ("forty one rooms free"). If rehearsals booked into it, the answers would drift a little further
# from the sheet with every run -- and the recording would be made on the most-drifted copy of all.
# So the runtime reads and writes `aether_hotel.live.db`, made from the shipped file on first use and
# gitignored; `scripts/reset_hotel_db.py` throws it away.
LIVE_DB_PATH = SHIPPED_DB_PATH.with_name("aether_hotel.live.db")


def _live_copy() -> Path:
    """The working copy, created -- or refreshed -- from the shipped database as needed.

    Refreshed when the shipped file is NEWER: a `git pull` or a schema change (`expand_hotel_db.py`
    added `table_bookings`) has to reach the running system, and a copy that silently lacked a table
    would fail on its first booking. The cost is that rehearsal bookings are dropped when that
    happens, which is the right trade -- stale structure is a crash, stale bookings are nothing.

    Copied to a temporary name and swapped in, so a second process starting at the same moment can
    never open a half-written file. If the swap is refused (on Windows, another process holding the
    old copy open), the existing copy is used rather than crashing a call over a refresh.
    """
    import shutil

    live, shipped = LIVE_DB_PATH, SHIPPED_DB_PATH
    if not shipped.exists():
        return live if live.exists() else shipped
    if live.exists() and live.stat().st_mtime >= shipped.stat().st_mtime:
        return live
    partial = live.with_name(live.name + ".partial")
    shutil.copy2(shipped, partial)
    try:
...
def dump_sql(path: Path | str | None = None) -> tuple[str, str]:
    """(schema, seed) as SQL text, generated FROM the database itself.

    `data/hotel_db/schema.sql` used to be a one-line note and `seed.sql` a hand-kept dump missing
    `hotel_policies` and `table_bookings` -- a second, older definition of a database whose whole
    design is that there is exactly one. Both are now generated by `scripts/dump_hotel_db.py`, and a
    test regenerates them and compares, so they cannot drift again.
    """
    import sqlite3 as _sqlite3

    target = Path(path) if path is not None else SHIPPED_DB_PATH
    con = _sqlite3.connect(f"file:{target.as_posix()}?mode=ro", uri=True)
    try:
        schema: list[str] = []
        seed: list[str] = []
        for statement in con.iterdump():
            if statement in ("BEGIN TRANSACTION;", "COMMIT;"):
                continue
            (seed if statement.startswith("INSERT INTO") else schema).append(statement)
    finally:
        con.close()
    header = ("-- GENERATED from data/aether_hotel.db by scripts/dump_hotel_db.py.\n"
